...
export class TitleScene extends Phaser.Scene {
    ...
    for (let x = 0; x < tilesX; x++) {
      for (let y = 0; y < tilesY; y++) {
        const bg = this.add.image(
          x * bgWidth,
          y * bgHeight,
          "refined_notebook_title_background"
        );
        bg.setOrigin(0, 0);
        bg.setDepth(-100);
      }
  ...
  createUI() {
    ...
    if (
      this.gameTitle.width / this.gameTitle.height >
      maxTitleWidth / maxTitleHeight
    ) {
  ...
  createInstructionText() {
    ...
    this.startText = this.add
      .text(screenWidth / 2, screenHeight * 0.65, "TAP TO START", {
...
Collect power-ups for special abilities!
    ...
    this.controlsDisplay = this.add
      .text(screenWidth / 2, screenHeight * 0.85, controlsText, {
        fontFamily: "SupercellMagic",
